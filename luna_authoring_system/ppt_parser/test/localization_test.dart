...
void main() {
  ...
  group('ModuleTextElements and TextElements Class Tests', () {
    ...
    Future<ModuleTextElements> getElementsFromPPTX(var pptx_name) async {
      ...
      Locale defaultLocale = Locale('en', 'US'); // Use Locale class
      ModuleTextElements moduleData = ModuleTextElements(prsTree, defaultLocale.toLanguageTag());
      return moduleData;
    }

    test('ModuleTextElements: A PPTX with 1 Text Box results in 1 token.', () async {
      ModuleTextElements moduleData = await getElementsFromPPTX("TextBox-HelloWorld.pptx");
      expect(moduleData.elements.length, 1);
    });

    test('TextElement: A PPTX with 1 Text box has expected strings and assigned UID', () async {
      ModuleTextElements moduleData = await getElementsFromPPTX("TextBox-HelloWorld.pptx");
    ...
    test('ModuleTextElements generateCSV - creates a CSV file in the specified directory', () async {
    ...
    test('ModuleTextElements: A PPTX with 3 Text Boxes results in 13 tokens.', () async {
      ModuleTextElements moduleData = await getElementsFromPPTX("Textboxes.pptx");
      expect(moduleData.elements.length, 3);
    });

    test('TextElement: A PPTX with 3 Text boxes has properly assigned Text Element strings, language, and UIDs.', () async {
      ModuleTextElements moduleData = await getElementsFromPPTX("Textboxes.pptx");
      TextElement? element = moduleData.elements[2];
      expect(element?.uid, 2);
      expect(element?.originalText, "Thing2");
      expect(element?.originalLanguageLocale, Locale('en', 'US').toLanguageTag());
    });
  });
}
