...
void main() {
  group('Unit Tests for Point2DPercentage', () {
    ...
    test('Throws ArgumentError when x is negative', () {
      expect(() => Point2DPercentage(-0.5, 0.5), throwsA(isA<ArgumentError>()));
    });
    test('Throws ArgumentError when y is negative', () {
      expect(() => Point2DPercentage(0.5, -0.5), throwsA(isA<ArgumentError>()));
    });

    // Ensures an error is thrown if x or y is greater than 1.
    test('Throws ArgumentError when x is greater than 1', () {
      expect(() => Point2DPercentage(1.2, 0.5), throwsA(isA<ArgumentError>()));
    });
    test('Throws ArgumentError when y is greater than 1', () {
      expect(() => Point2DPercentage(0.5, 1.5), throwsA(isA<ArgumentError>()));
    });
